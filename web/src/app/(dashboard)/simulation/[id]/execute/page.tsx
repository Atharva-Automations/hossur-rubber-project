/* eslint-disable react-hooks/exhaustive-deps */
/* eslint-disable @typescript-eslint/no-explicit-any */
'use client';

import api from '@/lib/api';
import QRCode from 'react-qr-code';
import { useState, useEffect } from 'react';
import { useParams } from 'next/navigation';

import { Header, PageContainer, Card } from '@/components/global';
import { Input } from '@/components/ui/input';
import { Button } from '@/components/ui/button';
import { toast } from '@/components/ui/use-toast';
import {
  Dialog,
  DialogContent,
  DialogFooter,
  DialogHeader,
  DialogTitle,
} from '@/components/ui/dialog';
import {
  Accordion,
  AccordionContent,
  AccordionItem,
  AccordionTrigger,
} from '@/components/ui/accordion';
import ProductQrModal from './components/modals/ProductQrModal';

const STEPS = ['QR Mode', 'Weighing', 'Execution', 'Completed'] as const;

// ------------------ TYPES ------------------
type NextWeighItem = {
  batchId: number;
  productExecutionId: number;
  productSequence: number;
  batchStepId: number;
  batchIngredientId: number;
  stepType: 'KNEADER' | 'MIXING';
  stepSequenceNumber: number;
  ingredientId: number;
  ingredientCode: string;
  ingredientName: string;
  binNumber: string | null;
  requiredWeight: number;
  unit: string;
  sequenceInStep: number;
  qrId?: string;
} | null;

type ExecutionStatusProduct = {
  productExecutionId: number;
  productSequence: number;
  status: string;
  weighingStartedAt?: string | null;
  weighingCompletedAt?: string | null;
  executionStartedAt?: string | null;
  executionCompletedAt?: string | null;
  steps: {
    stepExecutionId: number;
    batchStepId: number;
    stepType: 'KNEADER' | 'MIXING';
    sequenceNumber: number;
    status: 'PENDING' | 'READY' | 'IN_PROGRESS' | 'DONE';
    ingredientsAdded: number;
    ingredientsExpected: number;
  }[];
};

type ExecutionStatusResponse = {
  batchId: number;
  status: string;
  products: ExecutionStatusProduct[];
};

type StepDetails = {
  productExecutionId: number;
  productSequence: number;
  stepExecutionId: number;
  batchStepId: number;
  stepType: 'KNEADER' | 'MIXING';
  sequenceNumber: number;
  timerSeconds: number;
  remainingSeconds: number | null;
  temperature: number;
  pressure: number;
  rpm: number;
  stepStatus: 'PENDING' | 'READY' | 'IN_PROGRESS' | 'DONE';
  expectedIngredients: {
    batchIngredientId: number;
    stepNumber: number;
    stepType: 'KNEADER' | 'MIXING';
    ingredientCode: string;
    ingredientName: string;
    quantityPerUnit: number;
    unit: string;
    scanned: boolean;
  }[];
  executedIngredients: {
    qrId: string;
    ingredientCode: string;
    ingredientName: string;
  }[];
};

// -------------------------------------------

export default function ExecutionPage() {
  const { id } = useParams<{ id: string }>();
  const batchId = Number(id);

  // ---- WEIGHING ----
  const [weight, setWeight] = useState('');
  const [bulkList, setBulkList] = useState<any[]>([]);
  const [bulkScanInput, setBulkScanInput] = useState('');
  const [lastQr, setLastQr] = useState<string | null>(null);
  const [weighedList, setWeighedList] = useState<any[]>([]);
  const [currentItem, setCurrentItem] = useState<NextWeighItem>(null);
  const [qrMode, setQrMode] = useState<'SEQUENTIAL' | 'BULK' | null>(null);
  const [stepIndex, setStepIndex] = useState<0 | 1 | 2 | 3>(0);
  const [weighingCompleted, setWeighingCompleted] = useState(false);
  const [showWeightModal, setShowWeightModal] = useState(false);
  const [bulkScannedItem, setBulkScannedItem] = useState<any>(null);

  // ---- EXECUTION ----
  const [executionStatus, setExecutionStatus] =
    useState<ExecutionStatusResponse | null>(null);
  const [selectedProductId, setSelectedProductId] = useState<number | null>(
    null
  );
  const [currentStep, setCurrentStep] = useState<StepDetails | null>(null);
  const [scanInput, setScanInput] = useState('');
  const [timerRunning, setTimerRunning] = useState(false);
  const [remainingSeconds, setRemainingSeconds] = useState<number | null>(null);
  const [stepCompleted, setStepCompleted] = useState(false);
  const [productCompleted, setProductCompleted] = useState(false);

  // ---- QR ----
  const [qrModalOpen, setQrModalOpen] = useState(false);
  const [qrData, setQrData] = useState<any>(null);

  // ----------------------- LOAD WEIGHING -----------------------
  useEffect(() => {
    loadWeighedList();
  }, []);

  const loadWeighedList = async () => {
    const res = await api.get(`/batch/${batchId}/weighing/list`);
    setWeighedList(res.data || []);
  };

  // ----------------------- MODE SELECT -----------------------
  const handleModeSelect = async (mode: 'SEQUENTIAL' | 'BULK') => {
    setQrMode(mode);

    if (mode === 'BULK') {
      try {
        await api.post(`/batch/${batchId}/bulk/start`);
        const res = await api.get(`/batch/${batchId}/bulk/labels`);
        setBulkList(res.data || []);
        setStepIndex(1);
      } catch (err: any) {
        toast({
          title: 'Failed to start BULK mode',
          description: err?.response?.data?.message,
          variant: 'destructive',
        });
        setQrMode(null);
      }
      return;
    }

    // SEQUENTIAL
    try {
      await api.post(`/batch/${batchId}/start`);
      await loadNext();
      setStepIndex(1);
    } catch (err: any) {
      toast({
        title: 'Failed to start',
        description: err?.response?.data?.message,
        variant: 'destructive',
      });
      setQrMode(null);
    }
  };

  // ----------------------- NEXT INGREDIENT -----------------------
  const loadNext = async () => {
    const res = await api.get(`/batch/${batchId}/weighing/next`);
    const item = res.data || null;
    setCurrentItem(item);

    if (!item) {
      setWeighingCompleted(true);
      await loadWeighedList();
    }
  };

  const handleConfirmWeigh = async () => {
    const num = Number(weight);
    if (!num || num <= 0) {
      toast({
        title: 'Invalid weight',
        description: 'Enter valid weight',
        variant: 'destructive',
      });
      return;
    }

    // SEQUENTIAL
    if (qrMode === 'SEQUENTIAL' && currentItem) {
      try {
        const res = await api.post(`/batch/${batchId}/weigh`, {
          batchId,
          batchStepId: currentItem.batchStepId,
          batchIngredientId: currentItem.batchIngredientId,
          weight: num,
        });

        setLastQr(res.data.qrId);
        setCurrentItem({ ...currentItem, qrId: res.data.qrId });

        toast({
          title: 'Correct weight!',
          description: res.data.qrId,
        });

        setShowWeightModal(false);
        setWeight('');
      } catch (err: any) {
        toast({
          title: 'Weigh failed',
          description: err?.response?.data?.message,
          variant: 'destructive',
        });
      }
      return;
    }

    // BULK
    if (qrMode === 'BULK' && bulkScannedItem) {
      try {
        const res = await api.post(`/batch/${batchId}/bulk/weigh`, {
          qrId: bulkScannedItem.qrId,
          weight: num,
        });

        toast({ title: 'Saved', description: res.data.qrId });

        const refreshed = await api.get(`/batch/${batchId}/bulk/labels`);
        setBulkList(refreshed.data || []);

        setBulkScannedItem(null);
        setBulkScanInput('');
        setShowWeightModal(false);
        setWeight('');
      } catch (err: any) {
        toast({
          title: 'Bulk weigh failed',
          description: err?.response?.data?.message,
          variant: 'destructive',
        });
      }
    }
  };

  const handleNextIngredient = async () => {
    if (!currentItem?.qrId) {
      toast({ title: 'Scan not completed', variant: 'destructive' });
      return;
    }

    await loadNext();
    await loadWeighedList();
  };

  // ----------------------- EXECUTION PHASE -----------------------
  const handleStartExecutionPhase = async () => {
    try {
      await api.post(`/batch/${batchId}/start-execution`);
      setStepIndex(2);
    } catch (err: any) {
      toast({
        title: 'Cannot start execution',
        description: err?.response?.data?.message,
        variant: 'destructive',
      });
    }
  };

  // ----------- LOAD EXECUTION STATUS -----------
  const loadExecutionStatus = async () => {
    try {
      const res = await api.get(`/batch/${batchId}/execution/status`);
      const data: ExecutionStatusResponse = res.data;
      setExecutionStatus(data);
    } catch (err: any) {
      toast({
        title: 'Failed',
        description: err?.response?.data?.message,
        variant: 'destructive',
      });
    }
  };

  useEffect(() => {
    if (stepIndex === 2) loadExecutionStatus();
  }, [stepIndex]);

  // ------------------ PRODUCT CLICK HANDLER (UPDATED) ------------------
  const handleProductClick = async (product: ExecutionStatusProduct) => {
    // 1️⃣ If product steps are completed BUT QR not generated → show QR generation screen
    if (product.status === 'STEP_COMPLETED') {
      setSelectedProductId(product.productExecutionId);
      setProductCompleted(true);
      setCurrentStep(null);
      return;
    }

    // 2️⃣ If product is fully completed → QR should already exist → open modal
    if (product.status === 'PRODUCT_COMPLETED') {
      try {
        const qr = await api.get(
          `/batch/${batchId}/product/${product.productExecutionId}/qr`
        );
        setQrData(qr.data);
        setQrModalOpen(true);
      } catch {
        toast({
          title: 'QR not available',
          description: 'Generate product QR first.',
          variant: 'destructive',
        });
      }
      return;
    }

    // 3️⃣ Otherwise → open step execution UI
    setSelectedProductId(product.productExecutionId);
    await loadActiveStepForProduct(product.productExecutionId);
  };

  // ------------------ START PRODUCT EXECUTION ------------------
  const handleStartProductExecution = async (
    product: ExecutionStatusProduct
  ) => {
    if (!executionStatus) return;

    const someoneRunning = executionStatus.products.some(
      (p) => p.status === 'STEP_IN_PROGRESS'
    );
    if (someoneRunning) {
      toast({
        title: 'Another product is in execution',
        description: 'Finish that product first.',
        variant: 'destructive',
      });
      return;
    }

    try {
      await api.post(
        `/batch/${batchId}/product-execution/${product.productExecutionId}/start`,
        { productExecutionId: product.productExecutionId, batchStepId: null }
      );

      await loadExecutionStatus();
      await handleProductClick(product);
    } catch (err: any) {
      toast({
        title: 'Cannot start',
        description: err?.response?.data?.message,
        variant: 'destructive',
      });
    }
  };

  // ------------------ LOAD ACTIVE STEP DETAILS ------------------
  const loadActiveStepForProduct = async (productExecutionId: number) => {
    try {
      const res = await api.get(`/batch/${batchId}/execution/active-step`, {
        params: { productExecutionId },
      });

      const step: StepDetails = res.data;

      setProductCompleted(false);
      setCurrentStep(step);

      // DONE step
      if (step.stepStatus === 'DONE') {
        setTimerRunning(false);
        return;
      }

      // IN PROGRESS
      if (step.stepStatus === 'IN_PROGRESS') {
        const secs = step.remainingSeconds ?? step.timerSeconds;
        setRemainingSeconds(secs);
        setTimerRunning(true);
        return;
      }

      // READY state
      setRemainingSeconds(step.timerSeconds);
      setTimerRunning(false);
    } catch (err: any) {
      const msg = err?.response?.data?.message || '';

      // IMPORTANT: detect backend message
      if (msg.includes('All steps completed')) {
        setProductCompleted(true);
        setCurrentStep(null);
        return;
      }

      toast({
        title: 'Cannot load step',
        description: msg,
        variant: 'destructive',
      });
    }
  };

  // ------------------ SCAN INGREDIENT QR ------------------
  const handleScanExecutionQr = async () => {
    if (!currentStep || !scanInput.trim()) return;

    if (timerRunning) {
      toast({
        title: 'Step running',
        description: 'Cannot scan while machine is running.',
        variant: 'destructive',
      });
      return;
    }

    try {
      await api.post(`/batch/${batchId}/execution/scan`, {
        qrId: scanInput.trim(),
      });

      setScanInput('');
      await loadActiveStepForProduct(currentStep.productExecutionId);
      await loadExecutionStatus();
    } catch (err: any) {
      toast({
        title: 'Scan failed',
        description: err?.response?.data?.message,
        variant: 'destructive',
      });
    }
  };

  // ------------------ START STEP ------------------
  const handleStartStep = async () => {
    if (!currentStep) return;

    const allScanned = currentStep.expectedIngredients.every((i) => i.scanned);
    if (!allScanned) {
      toast({
        title: 'Incomplete',
        description: 'Scan all ingredients first.',
        variant: 'destructive',
      });
      return;
    }

    try {
      await api.post(
        `/batch/${batchId}/product-execution/${currentStep.productExecutionId}/steps/${currentStep.batchStepId}/start`
      );

      setStepCompleted(false);
      setRemainingSeconds(currentStep.timerSeconds);
      setTimerRunning(true);

      setCurrentStep((prev) =>
        prev ? { ...prev, stepStatus: 'IN_PROGRESS' } : prev
      );
      await loadExecutionStatus();
    } catch (err: any) {
      toast({
        title: 'Start failed',
        description: err?.response?.data?.message,
        variant: 'destructive',
      });
    }
  };

  // ------------------ COMPLETE STEP ------------------
  const handleCompleteStep = async () => {
    if (!currentStep || stepCompleted) return;

    try {
      await api.post(`/batch/${batchId}/execution/complete-step`, {
        productExecutionId: currentStep.productExecutionId,
        batchStepId: currentStep.batchStepId,
      });

      setTimerRunning(false);
      setStepCompleted(true);

      await new Promise((r) => setTimeout(r, 250));

      await loadExecutionStatus();
      await loadActiveStepForProduct(currentStep.productExecutionId);
    } catch (err: any) {
      toast({
        title: 'Complete failed',
        description: err?.response?.data?.message,
        variant: 'destructive',
      });
    }
  };

  // ------------------ TIMER ------------------
  useEffect(() => {
    if (!timerRunning || remainingSeconds === null) return;

    const id = setInterval(() => {
      setRemainingSeconds((prev) => {
        if (prev === null) return prev;
        if (prev <= 1) {
          clearInterval(id);
          setTimerRunning(false);
          handleCompleteStep();
          return 0;
        }
        return prev - 1;
      });
    }, 1000);

    return () => clearInterval(id);
  }, [timerRunning, remainingSeconds]);

  // ------------------ BACK ------------------
  const handleBackToProducts = () => {
    setSelectedProductId(null);
    setCurrentStep(null);
    setTimerRunning(false);
    setRemainingSeconds(null);
    setStepCompleted(false);
  };

  // ------------------ GENERATE PRODUCT QR ------------------
  const handleGenerateProductQr = async () => {
    try {
      const res = await api.post(
        `/batch/${batchId}/execution/finalize-product`,
        { productExecutionId: selectedProductId }
      );

      setQrData(res.data);
      setQrModalOpen(true);

      await loadExecutionStatus();
    } catch (err: any) {
      toast({
        title: 'Failed',
        description: err?.response?.data?.message,
        variant: 'destructive',
      });
    }
  };

  const isProductCompleted = (
    productSeq: number,
    items: any[],
    weighed: any[]
  ) => {
    return items.every((item) =>
      weighed.some(
        (w) =>
          w.batchIngredientId === item.batchIngredientId &&
          w.productSequence === productSeq
      )
    );
  };

  // ------------------ GENERATE BATCH QR ------------------
  const generateBatchQr = async () => {
    try {
      const res = await api.post(`/batch/${batchId}/finalize-batch`);
      setQrData(res.data);
      setQrModalOpen(true);
    } catch (err: any) {
      toast({
        title: 'Batch QR generation failed',
        description: err?.response?.data?.message,
        variant: 'destructive',
      });
    }
  };

  const fetchBatchQr = async () => {
    try {
      const res = await api.get(`/batch/${batchId}/qr`);
      setQrData(res.data);
      setQrModalOpen(true);
    } catch {
      toast({
        title: 'QR not available',
        description: 'Generate batch QR first.',
        variant: 'destructive',
      });
    }
  };

  // ⬇ AUTO-MOVE TO COMPLETED WHEN ALL PRODUCTS ARE DONE
  useEffect(() => {
    if (
      stepIndex === 2 &&
      executionStatus?.products?.every((p) => p.status === 'PRODUCT_COMPLETED')
    ) {
      setStepIndex(3);
    }
  }, [executionStatus, stepIndex]);

  // ------------------ PRODUCT LIST HELPERS ------------------
  const hasActiveProduct =
    executionStatus?.products.some((p) => p.status === 'STEP_IN_PROGRESS') ??
    false;

  const unifiedProducts = executionStatus?.products ?? [];

  // -------------------------------------------------------------------------
  // ------------------------------- RENDER UI -------------------------------
  // -------------------------------------------------------------------------

  return (
    <PageContainer>
      <Header
        title={`Batch Execution: ${id}`}
        description="Execute batch production steps and manage weighing"
        icon="🏭"
      />

      <div className="mt-8 max-w-4xl space-y-6">
        {/* Progress Steps */}
        <Card className="p-4">
          <div className="flex items-center gap-2 text-sm">
            {STEPS.map((label, idx) => (
              <ProgressStep
                key={label}
                active={stepIndex === idx}
                done={stepIndex > idx}
                label={label}
              >
                {idx < STEPS.length - 1 && <Dash />}
              </ProgressStep>
            ))}
          </div>
        </Card>

        {/* ---------------- QR MODE SELECT ---------------- */}
        {qrMode === null && stepIndex === 0 && (
          <Card className="p-6">
            <h3 className="text-lg font-semibold mb-4">
              Select QR printing mode
            </h3>
            <div className="grid gap-4 md:grid-cols-2">
              <ModeOption
                title="Sequential"
                description="Print 1 QR after each ingredient weigh"
                onClick={() => handleModeSelect('SEQUENTIAL')}
              />
              <ModeOption
                title="Bulk"
                description="Print all QRs upfront & weigh anytime"
                onClick={() => handleModeSelect('BULK')}
              />
            </div>
          </Card>
        )}

        {/* ---------------- WEIGHING SCREEN ---------------- */}
        {/* SEQUENTIAL MODE */}
        {stepIndex === 1 && qrMode === 'SEQUENTIAL' && currentItem && (
          <Card className="p-6 space-y-4">
            <div className="flex justify-between">
              <div className="text-sm space-y-1">
                <div>
                  <b>Product:</b> {currentItem.productSequence}
                </div>
                <div>
                  <b>Step:</b> {currentItem.stepType} (
                  {currentItem.stepSequenceNumber})
                </div>
                <div>
                  <b>Ingredient:</b> {currentItem.ingredientCode} —{' '}
                  {currentItem.ingredientName}
                </div>
                <div>
                  <b>Required:</b> {currentItem.requiredWeight}{' '}
                  {currentItem.unit}
                </div>
              </div>

              <div className="flex flex-col items-center">
                {lastQr ? (
                  <>
                    <QRCode value={lastQr} size={80} />
                    <p className="text-xs mt-1">{lastQr}</p>
                  </>
                ) : (
                  <p className="text-xs text-gray-400">QR will appear here</p>
                )}
              </div>
            </div>

            {!currentItem.qrId ? (
              <Button
                onClick={() => setShowWeightModal(true)}
                className="bg-blue-600"
              >
                Weigh Ingredient
              </Button>
            ) : (
              <Button
                onClick={handleNextIngredient}
                className="bg-blue-600 w-full"
              >
                Next Ingredient →
              </Button>
            )}
          </Card>
        )}

        {/* BULK MODE */}
        {stepIndex === 1 && qrMode === 'BULK' && (
          <Card className="p-6 space-y-6">
            <div>
              <h3 className="text-lg font-semibold">Scan Ingredient QR</h3>
              <p className="text-sm text-gray-500 mb-3">
                Enter or scan the QR printed for this ingredient.
              </p>

              <div className="flex gap-2">
                <Input
                  placeholder="Scan QR here"
                  value={bulkScanInput}
                  onChange={(e) => setBulkScanInput(e.target.value)}
                />

                <Button
                  onClick={async () => {
                    try {
                      const res = await api.post(
                        `/batch/${batchId}/bulk/scan`,
                        {
                          qrId: bulkScanInput.trim(),
                        }
                      );
                      setBulkScannedItem(res.data);
                      setLastQr(res.data.qrId);
                      toast({
                        title: 'QR recognized!',
                        description: res.data.ingredientCode,
                      });
                    } catch (err: any) {
                      toast({
                        title: 'Invalid QR',
                        description:
                          err?.response?.data?.message || 'Scan failed',
                        variant: 'destructive',
                      });
                    }
                  }}
                >
                  Scan
                </Button>
              </div>
            </div>

            {bulkScannedItem && (
              <div className="rounded border p-4 bg-gray-50 space-y-3">
                <div className="flex items-start justify-between">
                  <div className="text-sm space-y-1 flex-1">
                    <div>
                      <b>Product:</b> {bulkScannedItem.productSequence}
                    </div>
                    <div>
                      <b>Step:</b> {bulkScannedItem.stepType} (Seq{' '}
                      {bulkScannedItem.stepSequenceNumber})
                    </div>
                    <div>
                      <b>Ingredient:</b> {bulkScannedItem.ingredientCode} —{' '}
                      {bulkScannedItem.ingredientName}
                    </div>
                    <div>
                      <b>Bin:</b> {bulkScannedItem.binNumber ?? 'Not assigned'}
                    </div>
                    <div>
                      <b>Required Weight:</b> {bulkScannedItem.requiredWeight}{' '}
                      {bulkScannedItem.unit}
                    </div>
                  </div>

                  <div className="flex flex-col items-center flex-1">
                    <QRCode value={bulkScannedItem.qrId} size={90} />
                    <p className="mt-2 text-xs font-mono">
                      {bulkScannedItem.qrId}
                    </p>
                  </div>
                </div>

                <Button
                  className="bg-blue-600 mt-3"
                  onClick={() => setShowWeightModal(true)}
                >
                  Enter Weight
                </Button>
              </div>
            )}
          </Card>
        )}

        {/* QR HISTORY — SEQUENTIAL */}
        {stepIndex === 1 &&
          qrMode === 'SEQUENTIAL' &&
          weighedList.length > 0 && (
            <Card className="p-4 mt-6">
              <h3 className="text-md font-semibold mb-4">
                Weighed Ingredients
              </h3>

              <Accordion type="multiple" className="w-full">
                {Object.entries(
                  weighedList.reduce((groups, item) => {
                    const key = item.productSequence;
                    if (!groups[key]) groups[key] = [];
                    groups[key].push(item);
                    return groups;
                  }, {} as Record<number, typeof weighedList>)
                ).map(([productSeq, items]) => (
                  <AccordionItem
                    key={productSeq}
                    value={`product-${productSeq}`}
                  >
                    <AccordionTrigger className="text-sm font-medium">
                      Product {productSeq}
                    </AccordionTrigger>

                    <AccordionContent>
                      <div className="space-y-3 mt-2">
                        {(items as typeof weighedList).map((w) => (
                          <div
                            key={w.id}
                            className="flex justify-between items-center border rounded p-3 text-sm bg-white shadow-sm"
                          >
                            <div>
                              <b>{w.ingredientCode}</b> — {w.ingredientName}
                              <div className="text-xs text-gray-500">
                                {w.stepType} (Seq: {w.productSequence})
                              </div>
                            </div>

                            <div className="font-mono text-blue-600 text-xs">
                              {w.qrId}
                            </div>
                          </div>
                        ))}
                      </div>
                    </AccordionContent>
                  </AccordionItem>
                ))}
              </Accordion>
            </Card>
          )}

        {/* BULK QR LABEL LIST */}
        {stepIndex === 1 && qrMode === 'BULK' && bulkList.length > 0 && (
          <Card className="p-4 mt-6">
            <h3 className="text-md font-semibold mb-4">Bulk QR Labels</h3>

            <Accordion type="multiple" className="w-full">
              {Object.entries(
                bulkList.reduce((groups, item) => {
                  const key = item.productSequence;
                  if (!groups[key]) groups[key] = [];
                  groups[key].push(item);
                  return groups;
                }, {} as Record<number, Array<(typeof bulkList)[number]>>)
              ).map(([productSeq, items]) => (
                <AccordionItem key={productSeq} value={`bulk-${productSeq}`}>
                  <AccordionTrigger className="text-sm font-medium flex items-center gap-2">
                    {isProductCompleted(
                      Number(productSeq),
                      items as Array<any>,
                      weighedList as Array<any>
                    ) && <span className="text-green-600 text-lg">✔</span>}
                    Product {productSeq}
                  </AccordionTrigger>

                  <AccordionContent>
                    <div className="space-y-3 mt-2">
                      {(items as Array<any>).map((w) => (
                        <div
                          key={w.qrId}
                          className="flex justify-between items-center border rounded p-3 text-sm bg-white shadow-sm"
                        >
                          <div className="flex items-center gap-3">
                            {w.isWeighed ? (
                              <span className="text-green-600 text-lg">✔</span>
                            ) : (
                              <span className="inline-block w-4 h-4" />
                            )}
                            <div>
                              <b>{w.ingredientCode}</b> — {w.ingredientName}
                              <div className="text-xs text-gray-500">
                                {w.stepType} (Seq {w.stepSequenceNumber})
                              </div>
                            </div>
                          </div>

                          <span className="font-mono text-blue-600 text-xs">
                            {w.qrId}
                          </span>
                        </div>
                      ))}
                    </div>
                  </AccordionContent>
                </AccordionItem>
              ))}
            </Accordion>
          </Card>
        )}

        {/* After weighing complete */}
        {stepIndex === 1 && qrMode && weighingCompleted && (
          <Card className="p-6 text-center">
            <h2 className="text-xl text-green-700 font-semibold">
              All Ingredients Weighed!
            </h2>
            <p className="text-gray-600 mb-4">
              You can now start executing the batch.
            </p>
            <Button className="bg-blue-600" onClick={handleStartExecutionPhase}>
              Start Execution →
            </Button>
          </Card>
        )}

        {/* ---------------- EXECUTION SCREEN ---------------- */}
        {stepIndex === 2 && (
          <Card className="p-6 space-y-6">
            {selectedProductId !== null && productCompleted && (
              <Card className="p-6 text-center space-y-4">
                <h2 className="text-xl font-semibold text-green-700">
                  All steps completed for Product{' '}
                  {
                    executionStatus?.products.find(
                      (p) => p.productExecutionId === selectedProductId
                    )?.productSequence
                  }
                </h2>

                <p className="text-gray-700">
                  You can now generate the final Product QR.
                </p>

                <Button
                  className="bg-green-600 w-full"
                  onClick={handleGenerateProductQr}
                >
                  Generate Product QR →
                </Button>

                <Button
                  variant="outline"
                  className="w-full"
                  onClick={handleBackToProducts}
                >
                  ← Back to Products
                </Button>
              </Card>
            )}

            {/* ---------------- PRODUCT LIST ---------------- */}
            {selectedProductId == null ? (
              <>
                <h3 className="text-lg font-semibold mb-2">
                  Products in this Batch
                </h3>

                {unifiedProducts.length === 0 ? (
                  <p className="text-sm text-gray-500">Loading...</p>
                ) : (
                  <div className="space-y-2">
                    {unifiedProducts.map((p) => {
                      const isCompleted =
                        p.status === 'STEP_COMPLETED' ||
                        p.status === 'PRODUCT_COMPLETED';

                      return (
                        <div
                          key={p.productExecutionId}
                          className={`flex items-center justify-between border rounded p-3 text-sm ${
                            isCompleted ? 'bg-green-50' : 'bg-white'
                          }`}
                        >
                          <div>
                            <div className="font-semibold">
                              Product {p.productSequence}
                            </div>
                            <div className="text-xs text-gray-500">
                              Status: {p.status.replace(/_/g, ' ')}
                            </div>
                          </div>

                          <div className="flex gap-2">
                            <Button
                              variant="outline"
                              size="sm"
                              onClick={() => handleProductClick(p)}
                            >
                              Open
                            </Button>
                            {p.status === 'WEIGHING_COMPLETED' && (
                              <Button
                                size="sm"
                                disabled={hasActiveProduct}
                                onClick={() => handleStartProductExecution(p)}
                              >
                                Start
                              </Button>
                            )}

                            {(() => {
                              const qrAvailable =
                                p.status === 'PRODUCT_COMPLETED';
                              return (
                                <Button
                                  size="sm"
                                  variant="outline"
                                  disabled={!qrAvailable}
                                  onClick={async () => {
                                    try {
                                      const qr = await api.get(
                                        `/batch/${batchId}/product/${p.productExecutionId}/qr`
                                      );
                                      setQrData(qr.data);
                                      setQrModalOpen(true);
                                    } catch {
                                      toast({
                                        title: 'QR not available',
                                        description:
                                          'Generate product QR first.',
                                        variant: 'destructive',
                                      });
                                    }
                                  }}
                                >
                                  QR
                                </Button>
                              );
                            })()}
                          </div>
                        </div>
                      );
                    })}
                  </div>
                )}
              </>
            ) : !productCompleted && currentStep ? (
              <>
                {/* EXECUTION STEP UI (unchanged) */}
                {/* HEADER */}
                <div className="flex justify-between items-start">
                  <div className="space-y-1 text-sm">
                    <div>
                      <b>Product:</b> {currentStep.productSequence}
                    </div>
                    <div>
                      <b>Machine:</b> {currentStep.stepType}
                    </div>
                    <div>
                      <b>Step No:</b> {currentStep.sequenceNumber}
                    </div>
                  </div>

                  <div className="text-xs text-gray-600 text-right">
                    <div>
                      Timer: {remainingSeconds ?? currentStep.timerSeconds}s
                    </div>
                    <div>Temp: {currentStep.temperature}°C</div>
                    <div>Pressure: {currentStep.pressure} bar</div>
                    <div>RPM: {currentStep.rpm}</div>
                    <div>Status: {currentStep.stepStatus}</div>
                  </div>
                </div>

                {/* SCAN INGREDIENT */}
                <div>
                  <label className="block text-sm font-medium mb-1">
                    Scan Ingredient QR
                  </label>
                  <div className="flex gap-2">
                    <Input
                      placeholder="Scan QR here"
                      value={scanInput}
                      onChange={(e) => setScanInput(e.target.value)}
                      disabled={timerRunning}
                      onKeyDown={(e) => {
                        if (e.key === 'Enter') handleScanExecutionQr();
                      }}
                    />
                    <Button
                      disabled={timerRunning}
                      onClick={handleScanExecutionQr}
                    >
                      Scan
                    </Button>
                  </div>
                </div>

                {/* EXPECTED INGREDIENTS */}
                <div>
                  <h4 className="font-semibold mb-2">Expected Ingredients</h4>
                  <div className="space-y-2">
                    {Object.entries(
                      currentStep.expectedIngredients.reduce((acc, ing) => {
                        if (!acc[ing.stepNumber]) acc[ing.stepNumber] = [];
                        acc[ing.stepNumber].push(ing);
                        return acc;
                      }, {} as Record<number, StepDetails['expectedIngredients']>)
                    ).map(([stepNo, list]) => (
                      <div
                        key={stepNo}
                        className={`border rounded p-3 ${
                          Number(stepNo) === currentStep.sequenceNumber
                            ? 'border-blue-500 bg-blue-50'
                            : ''
                        }`}
                      >
                        <div className="text-sm font-medium mb-1">
                          Step {stepNo}
                        </div>

                        <div className="space-y-1">
                          {list.map((ing) => (
                            <div
                              key={ing.batchIngredientId}
                              className="flex items-center justify-between text-sm"
                            >
                              <div className="flex items-center gap-2">
                                {ing.scanned ? (
                                  <span className="text-green-600 text-lg">
                                    ✔
                                  </span>
                                ) : (
                                  <span className="inline-block w-4 h-4 rounded-full border" />
                                )}
                                <div>
                                  <b>{ing.ingredientCode}</b> —{' '}
                                  {ing.ingredientName}
                                  <div className="text-xs text-gray-500">
                                    Required: {ing.quantityPerUnit} {ing.unit}
                                  </div>
                                </div>
                              </div>
                            </div>
                          ))}
                        </div>
                      </div>
                    ))}
                  </div>
                </div>

                {/* GENERATE PRODUCT QR */}
                {currentStep.stepStatus === 'DONE' &&
                  executionStatus?.products?.find(
                    (p) => p.productExecutionId === selectedProductId
                  )?.status === 'STEP_COMPLETED' && (
                    <Button
                      className="bg-green-600 text-white"
                      onClick={handleGenerateProductQr}
                    >
                      Generate Product QR →
                    </Button>
                  )}

                {/* ACTION BUTTONS */}
                <div className="flex gap-3 mt-4">
                  <Button variant="outline" onClick={handleBackToProducts}>
                    ← Products
                  </Button>

                  <Button
                    className="flex-1 bg-blue-600"
                    onClick={handleStartStep}
                    disabled={
                      !currentStep ||
                      currentStep.stepStatus !== 'READY' ||
                      !currentStep.expectedIngredients.every(
                        (i) => i.scanned
                      ) ||
                      timerRunning
                    }
                  >
                    Start Step
                  </Button>
                </div>
              </>
            ) : (
              <p>Loading…</p>
            )}
          </Card>
        )}

        {/* ---------------- STEP 3 — COMPLETED (BATCH QR) ---------------- */}
        {stepIndex === 3 && (
          <Card className="p-6 space-y-6 text-center">
            <h2 className="text-xl font-semibold text-green-700">
              All Products Completed!
            </h2>

            <p className="text-gray-600">
              You can now generate the final Batch QR.
            </p>

            <Button className="bg-green-600 w-full" onClick={generateBatchQr}>
              Generate Batch QR →
            </Button>

            <Button className="w-full" variant="outline" onClick={fetchBatchQr}>
              View Batch QR
            </Button>
          </Card>
        )}

        {/* WEIGHT MODAL */}
        <Dialog open={showWeightModal} onOpenChange={setShowWeightModal}>
          <DialogContent className="bg-white">
            <DialogHeader>
              <DialogTitle>Enter Weighed Value</DialogTitle>
            </DialogHeader>

            <Input
              type="number"
              value={weight}
              onChange={(e) => setWeight(e.target.value)}
            />

            <DialogFooter>
              <Button
                variant="outline"
                onClick={() => setShowWeightModal(false)}
              >
                Cancel
              </Button>
              <Button onClick={handleConfirmWeigh}>Check Weight</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>

        {/* PRODUCT QR MODAL */}
        <ProductQrModal
          open={qrModalOpen}
          onClose={() => setQrModalOpen(false)}
          qrData={qrData ?? undefined}
        />
      </div>
    </PageContainer>
  );
}

// ------------------ SMALL COMPONENTS ------------------

function ProgressStep({ active, done, label, children }: any) {
  return (
    <div className="flex flex-1 items-center gap-2">
      <span
        className={`h-3 w-3 rounded-full ${
          done ? 'bg-green-500' : active ? 'bg-blue-600' : 'bg-gray-300'
        }`}
      />
      <span className={active ? 'text-blue-700 font-medium' : 'text-gray-600'}>
        {label}
      </span>
      {children}
    </div>
  );
}

function Dash() {
  return <div className="h-px flex-1 bg-gray-200" />;
}

function ModeOption({ title, description, onClick, disabled }: any) {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className={`rounded border p-4 text-left ${
        disabled ? 'opacity-50' : 'hover:border-blue-500'
      }`}
    >
      <div className="font-semibold">{title}</div>
      <p className="text-sm text-gray-500">{description}</p>
    </button>
  );
}
